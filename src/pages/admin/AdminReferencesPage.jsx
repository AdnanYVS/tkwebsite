  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { error: dbError } = await supabase
        .from('reference_logos')
        .insert({ name, logo_url: publicUrl, sort_order: logos.length });
      if (dbError) throw dbError;
      toast({ title: 'Başarılı', description: 'Logo eklendi.', variant: 'success' });
      setFile(null);
      setName('');
      ...
      <Card className="shadow-xl border-gray-200">
        ...
        <CardContent>
          <div className="flex items-center gap-4 mb-6">
            ...
            <Input
              type="text"
              placeholder="Firma/Logo Adı (isteğe bağlı)"
              value={name}
          ...
          {loading ? (
            ...
            <ul className="space-y-4">
              {logos.map((logo, idx) => (
                <li key={logo.id} className="flex items-center gap-4 bg-gray-50 rounded-lg p-3 shadow-sm">
